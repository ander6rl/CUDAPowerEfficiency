 ...
 * Edited by: Rebecca Anderson with help from Dr. Mike Lam
 **/
...
// The template kernel function signature changed to include blockdim as a parameter
// can be instantiated with different data types and configuration parameters
// blockdim is now a regular function parameter instead of a template parameter
...
// signature changed to inlcude blockSize as a parameter, but not used
...
// changed to include blockSize as an argument
...
void runbench(double *cd, long size, bool doHalfs, int blockSize){
	...
	// third parameter for BLOCK_SIZE passed to kernel function
...
extern "C" void mixbenchGPU(double *c, long size, int blockSize, bool runWarmup){
        ...
        if(runWarmup){
                printf("Warm-up run\n");
                // Error identigier blockSize is undefined
